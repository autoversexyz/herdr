...
`herdr api report-activity JSON` 通过 `activity.report` 替换某个来源的只读任务数据。参数为 `source`、单调递增的 `seq`、`ttl_ms`（1–120000）、`tasks`（最多100项）及 `omitted`。每项包含 `id`、`request_id`、`harness`、`workspace`、`status`、`progress`，以及可选的 `model`、`effort`、`elapsed_seconds`。最多8个来源；来源名称最多64个UTF-8字节，任务文本最多256个字节且不含控制字符。空列表或TTL到期会清除显示。

`session.snapshot` 和稳定JSON客户端快照通过可选的 `activity` 字段提供这些数据。旧客户端可忽略它；无效或不受支持的扩展不会中断核心连接。固定的二进制协议保持不变。展开的代理面板显示只读无头任务区及省略数量；断开的端点显示过期观测。带 `hygiene_kind=durable` 元数据的原生行标记为 durable。报告不会创建窗格、代理、输入目标或结果确认。
