...
`herdr api report-activity JSON` は `activity.report` を通して、発行元ごとの表示用タスクを置き換えます。パラメーターは `source`、単調増加する `seq`、`ttl_ms`（1–120000）、`tasks`（最大100件）、`omitted` です。各タスクは `id`、`request_id`、`harness`、`workspace`、`status`、`progress` と、省略可能な `model`、`effort`、`elapsed_seconds` を持ちます。発行元は最大8個、名前は64 UTF-8バイト以内、各テキストは制御文字を含まず256バイト以内です。空のリストまたはTTLの期限切れで表示を消去します。

`session.snapshot` と安定版JSONシェルスナップショットの省略可能な `activity` フィールドで配信します。古いクライアントは無視でき、未対応・不正な拡張でも基本接続は維持されます。固定バイナリ形式は変更しません。展開したエージェントパネルに読み取り専用のヘッドレス欄と省略件数を表示し、切断した端末の情報は古い観測として示します。`hygiene_kind=durable` メタデータ付きのネイティブ行には durable を表示します。この報告はペイン、エージェント、入力先、結果の確認応答を作成しません。
